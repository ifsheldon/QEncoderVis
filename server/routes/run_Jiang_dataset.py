...
def run_Jiang_dataset():
    ...
    result1 = np.array([float(a.item()) + float(b.item()) for a, b in zip(target_probs_list[:, 0], target_probs_list[:, 1])])
    result2 = np.array([float(a.item()) + float(b.item()) for a, b in zip(target_probs_list[:, 2], target_probs_list[:, 3])])
    ...
    result3 = [x.item() for x in (result2 - result1)]
