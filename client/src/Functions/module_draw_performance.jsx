...
function Module_draw_performance(props) {
    ...
    function draw_func(){
        ...
        let padding = { top: size*0.02, left: size*0.06, bottom: size*0.005, right: size*0.025 };
        ...
        let yScaleLoss = d3.scaleLinear()
            .domain([d3.min(dataset['loss']), d3.max(dataset['loss'])])
            .range([axis_height, 0]);


        let yScaleAccuracy = d3.scaleLinear()
            .domain([d3.min(dataset['accuracy']), d3.max(dataset['accuracy'])])
            .range([axis_height, 0]);


        let dataLoss = d3.range(dataset['epoch_number']).map((d,i)=>[d, dataset['loss'][d]])
        let dataAcc = d3.range(dataset['epoch_number']).map((d,i)=>[d, dataset['accuracy'][d]])
        ...
        let makeLossLine = d3.line()
            .x(d => xScale(d[0]))
            .y(d => yScaleLoss(d[1]))

        let makeAccLine = d3.line()
            .x(d => xScale(d[0]))
        ...
        gLoss.append('g')
            .call(d3.axisLeft(yScaleLoss).tickValues([d3.min(dataset['loss']), d3.max(dataset['loss'])]))
        ...
        gAcc.append('g')
            .call(d3.axisLeft(yScaleAccuracy).tickValues([d3.min(dataset['accuracy']), d3.max(dataset['accuracy'])]))
            .attr('transform', `translate(${-3}, 0)`)
        ...
        gLoss.append('text')
            ...
            .each(function() {
                ...
                let lines = ['Loss:', `${dataset['loss'][dataset['loss'].length-1].toFixed(3)}`]; // Each item represents a line
                lines.forEach(function(line, i) {
                    text.append('tspan') // Adding a tspan for each line
        ...
        gAcc.append('text')
            ...
            .each(function() {
                let text = d3.select(this);
                let lines = ['Acc:', `${dataset['accuracy'][dataset['accuracy'].length-1].toFixed(3)}`]; // Each item represents a line
                lines.forEach(function(line, i) {
                    text.append('tspan') // Adding a tspan for each line
                        .attr('x', 0) // Align with the starting x position
                        .attr('dy', i ? '1.2em' : 0) // Move subsequent lines down
